...
{
	// Level stuff
	level = Level( "levels/tutorial - step 1.lvl" ); // TODO: How do we choose level?
	step = 0;
	for ( std::vector<Object*>::iterator it = level.objects.begin(); it != level.objects.end(); ++it )
	{
		if ( *it != NULL )
		{
			...
			// This sets all trucks to non-active. This is unique to the tutorial
			else if ( (**it).type == Object::TRUCK )
			{
			    ( *(Truck*)(*it) ).active = false;
			}
		}
	}
	...
	pbuttonQuit.SetPosition(
		App.GetWidth() - pbuttonQuit.GetImage()->GetWidth() / 2.0f - 5,
		pbuttonQuit.GetImage()->GetHeight() / 2.0f + 5 );
...
/*			sf::Vector2f mappos = TransformScreenToMap( mousepos );
	...
	if (step == 0)
	{
	    str_howto.SetText("Welcome to Warehouse Panic! Here you will learn how to play.\nClick to continue");
	}
	else if (step == 1)
	{
	    str_howto.SetText("On the screen you should see three things:\n\t1. A truck\n\t2. A gate\n\t3. A long line of conveyor belts.\nWell, that's it! Nah, more coming soon");
	}
}
...
// Helper functions
sf::Vector2f SceneHowToPlay::TransformScreenToMap( sf::Vector2f& pos )
{
...
sf::Vector2f SceneHowToPlay::TransformMapToScreen( sf::Vector2f& pos )
{
	static float cosval = cos( M_PI / 4.0 );
	static float sinval = sin( M_PI / 4.0 );
...
float SceneHowToPlay::GetDistanceSQ( sf::Vector2f pos1, sf::Vector2f pos2 )
{
	sf::Vector2f diff( pos1.x - pos2.x, pos1.y - pos2.y );
	return diff.x * diff.x + diff.y * diff.y;
}

Object* SceneHowToPlay::FindClickedObject( sf::Vector2f& mappos )
{
...
bool SceneHowToPlay::DoCrate( Crate& crate )
{
...
bool SceneHowToPlay::DoTruck( Truck& truck )
{
	if ( truck.active )
	{
		if ( truck.interval <= 0 )
